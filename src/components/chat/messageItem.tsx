...
const MessageItemWrapper = styled.div<{ $isMine: boolean }>`
  display: flex;
  flex-direction: ${({ $isMine: isMine }) => (isMine ? 'row-reverse' : 'row')};
  gap: 8px;
  margin-bottom: 16px;
`;
...
const MessageContent = styled.div<{ $isMine: boolean }>`
  ...
  align-items: ${({ $isMine: isMine }) => (isMine ? 'flex-end' : 'flex-start')};
`;
...
const MessageBubble = styled.div<{ $isMine: boolean }>`
  background-color: ${({ $isMine: isMine }) =>
    isMine ? '#007AFF' : '#f0f0f0'};
  color: ${({ $isMine: isMine }) => (isMine ? '#fff' : '#000')};
  padding: 8px 12px;
  border-radius: 16px;
  max-width: 70%;
